...
object ChineseCalendar {
  ...
  def toGregorianCalendar(date: ChineseDate): GregorianCalendar = {
    ...
    val firstDay = eraMap(monarchEra) match {
      case ((start, table), ad) => 
        table(ad - start + yearOffset) match {
          case Year(fd, _) => fd
        }
    }
  ...
  def parseDate(s: String): ChineseDate = {
    var dayOfMonth = ""
  ...
  def parseMonth(s: String, dayOfMonth: String): ChineseDate = {
    var month = ""
    var endIndex = s.length
    if (s.endsWith("月")) {
  ...
  def parseYear(s: String, month: String, dayOfMonth: String): ChineseDate = {
